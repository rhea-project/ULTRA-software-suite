...
def turbofan_ductedfan_sizing(turbofan,mach_number = None, altitude = None, delta_isa = 0, conditions = None):  
    """ create and evaluate a turbofan-ducted fan network
    """
    ...
    inlet_nozzle_DF           = turbofan.inlet_nozzle_DF
    fan_DF                    = turbofan.fan_DF
    fan_nozzle_DF             = turbofan.fan_nozzle_DF
    thrust_DF                 = turbofan.thrust_DF
    ...
    turbofan.design_thrust = thrust.total_design * (1-thrust.hybridization)
    ...
    turbofan.sealevel_static_thrust_jet = results_sls.thrust_force_vector[0,0] / number_of_engines
    ...
    #link the fan nozzle to the fan
    fan_nozzle_DF.inputs.stagnation_temperature               = fan_DF.outputs.stagnation_temperature
    fan_nozzle_DF.inputs.stagnation_pressure                  = fan_DF.outputs.stagnation_pressure
    ...
    thrust_DF.inputs.number_of_engines                        = number_of_engines
    thrust_DF.inputs.bypass_ratio                             = bypass_ratio
    thrust_DF.inputs.total_temperature_reference              = fan_nozzle_DF.outputs.stagnation_temperature
    thrust_DF.inputs.total_pressure_reference                 = fan_nozzle_DF.outputs.stagnation_pressure
    thrust_DF.inputs.flow_through_core                        = 0.
    thrust_DF.inputs.flow_through_fan                         = 1.
    
    #nonexistant components used to run thrust
    thrust_DF.inputs.core_exit_velocity                       = 0.
    thrust_DF.inputs.core_area_ratio                          = 0.
    thrust_DF.inputs.core_nozzle                              = Data()
    thrust_DF.inputs.core_nozzle.velocity                     = 0.
    thrust_DF.inputs.core_nozzle.area_ratio                   = 0.
    thrust_DF.inputs.core_nozzle.static_pressure              = 0.                                                                                                                
    
    #compute the trust
    thrust_DF.size(conditions)
    mass_flow  = thrust_DF.mass_flow_rate_design
    
    #update the design thrust value
    turbofan.design_thrust1 = thrust_DF.total_design * thrust.hybridization
    ...
    state_sls.numerics = Data()
    state_sls.conditions = conditions_sls   
    results_sls1 = turbofan.evaluate_thrust(state_sls)

    turbofan.sealevel_static_thrust_DF = results_sls1.thrust_force_vector[0,0] / number_of_engines


    turbofan.sealevel_static_thrust = (results_sls.thrust_force_vector[0,0] + results_sls1.thrust_force_vector[0,0])/ number_of_engines 
