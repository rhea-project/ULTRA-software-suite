...
def turboprop_sizing(turboprop,mach_number = None, altitude = None, delta_isa = 0, conditions = None):  
    """ create and evaluate a turboprop network
    ...
    gearbox                   = turboprop.gearbox
    propeller                 = turboprop.propeller
    thrust                    = turboprop.thrust
    number_of_engines         = turboprop.number_of_engines
    ...
    thrust.inputs.low_pressure_turbine                     = low_pressure_turbine.outputs
    thrust.inputs.high_pressure_turbine                    = high_pressure_turbine.outputs
    thrust.inputs.low_pressure_compressor                  = low_pressure_compressor.outputs
    thrust.inputs.high_pressure_compressor                 = high_pressure_compressor.outputs   
    thrust.inputs.gearbox                                  = gearbox
    thrust.inputs.propeller                                = propeller

    thrust.size_turboprop(conditions,propeller.efficiency_map,propeller.max_altitude)
    
    #update the design thrust value
    turboprop.design_thrust = thrust.total_design *1e3              # Design turboprop power in W
    ...
    conditions_sls.freestream.isentropic_expansion_factor = np.atleast_1d(turboprop.working_fluid.compute_gamma(T,p))
    conditions_sls.freestream.Cp                          = np.atleast_1d(turboprop.working_fluid.compute_cp(T,p))
    conditions_sls.freestream.R                           = np.atleast_1d(turboprop.working_fluid.gas_specific_constant)
    ...
    results_sls          = turboprop.evaluate_thrust(state_sls)

    # Size the propeller using the Cubic spline method

    rho = conditions_sls.freestream.density[0][0]
    Ad  = 0.25 * np.pi * (2 * propeller.tip_radius)**2
    Asp = 0.25 * np.pi * (2 * propeller.hub_radius)**2

    turboprop.sealevel_static_thrust  =  0.85 * results_sls.power[0][0]**(2/3) * (2*rho*Ad)**(1/3) * (1-Asp/Ad)/ number_of_engines
