...
def hybrid_turbofan_sizing(turbofan,mach_number = None, altitude = None, delta_isa = 0, conditions = None):  
    """ create and evaluate a gas turbine network
        also accounts for a possibility to have a parallel hybrid energy network
    """
    ...
    # Update the turbofan thrust based on the degree of hybridization
    thrust.total_design = thrust.total_design
    ...
    # Dummy time for the electric motor
    time_list     = [0, 0.5]
    time_int_list = [0.5]
    state_sls.numerics.time = time_list
    state_sls.numerics.time = Data()
    state_sls.numerics.time.integrate = time_int_list

    results_sls = turbofan.evaluate_thrust(state_sls)

    turbofan.sealevel_static_thrust_jet = results_sls.thrust_force_vector[0,0]  * (1 - thrust.hybridization)/ number_of_engines

    # Calculate the electric motor maximum power in Watts
    Area = 0.25*np.pi*(turbofan.nacelle_diameter_electric**2-turbofan.hub_diameter_electric**2) 
    turbofan.max_power_electric = (results_sls.thrust_force_vector[0,0] * thrust.hybridization)**1.5/(2*conditions_sls.freestream.density[0,0]*Area)**(1/3)
