...
from SUAVE.Methods.Weights.FLOPS.Propulsion_systems.gearbox_weight                      import weight_gearbox                   as weight_gearbox 
from SUAVE.Methods.Weights.FLOPS.Propulsion_systems.propeller_weight                    import weight_propeller                 as weight_propeller
...
def empty(vehicle):
    ...
    if propulsor_name=='turbofan' or propulsor_name=='Turbofan':
        ...
        if propulsors.thrust.hybridization != 1:
        ...
        # Thrust_reversers
        if propulsors.thrust_reverser== True and propulsors.thrust.hybridization != 1:
        ...
        if propulsors.thrust.hybridization != 0:
            ...
            power_sls    = propulsors.motor.power/1000              # Sea-level electric engine power in kW
            P_W_inv      = propulsors.inverter.power_to_weight   

            # Total motor and inverter weights
            if propulsors.motor.tag == 'cryo-cooled motor':
                wt_engine2 = power_sls / T_W + num_el_eng * 27.5*propulsors.motor.cooling_power_loss*np.exp(-1.225*np.log10(propulsors.motor.cooling_power_loss))
            else:
                instal_penalty  = propulsors.installation_weight_penalty
                wt_engine2      = instal_penalty*power_sls / T_W          # Assume installation weight factor of the motor weight of 2
        ...
        # Engine
        num_eng   = propulsors.number_of_engines
        if propulsor_name =='turboprop':
            P_W       = propulsors.engine.power_to_weight    
            power_sls = propulsors.engine.sea_level_power               # power-to-weight ratio in kW/kg 
            # Gearbox
            wt_gearbox = weight_gearbox(propulsors.propeller,propulsors.gearbox,propulsors.engine,num_eng,power_sls)       
        else:
            P_W_inv    = propulsors.inverter.power_to_weight  
            P_W        = propulsors.motor.power_to_weight    
            power_sls  = propulsors.motor.sea_level_power               # power-to-weight ratio in kW/kg  
            # Gearbox
            wt_gearbox = weight_gearbox(propulsors.propeller,propulsors.gearbox,propulsors.motor,num_eng,power_sls)
                   
        wt_eng_misc     = propulsors.miscellaneous_weight 
        instal_penalty  = propulsors.installation_weight_penalty

        if propulsor_name == 'propeller cryo-electric':
            num_el_eng = num_eng
            wt_engine = instal_penalty*(power_sls / P_W + 27.5*propulsors.motor.cooling_power_loss*np.exp(-1.225*np.log10(propulsors.motor.cooling_power_loss))) * num_el_eng
        else:
            wt_engine = instal_penalty*power_sls / P_W * num_eng        # Assume installation weight of the motor weight
        ...
        # Propeller
        wt_propeller = weight_propeller(propulsors.propeller,power_sls,vehicle.cruise_mach,num_eng)

    wt_propulsion = wt_engine + wt_eng_misc + wt_thrust_rev + wt_gearbox + wt_propeller
    ...
    if 'horizontal_stabilizer' not in vehicle.wings:
        ...
        NHORT     = vehicle.wings['horizontal_stabilizer'].number_of_wings
        
        wt_tail_horizontal = weight_horizontal_tail(NHORT,S_h,TOW,lambda_h,Nult,Mc,p_h,p_0,vehicle.type)   
    ...
    if propulsors.type == 'hybrid':
        if propulsors.thrust.hybridization == 0:
            ...
            wt_air_ind = weight_nac(vehicle.type,num_jet_eng,thrust_sls_jet/num_jet_eng,thrust_base_jet/num_jet_eng,num_jet_eng_f,l_fus,h_fus,l_eng_jet,d_eng_jet)
    elif propulsors.type == 'propeller electric' or propulsors.type == 'turboprop' or propulsors.type == 'propeller cryo-electric':
        wt_air_ind = 0.0635 * power_sls * 1.34102 * num_eng
    elif propulsors.type == 'electric' or propulsors.type == 'cryo-electric':
