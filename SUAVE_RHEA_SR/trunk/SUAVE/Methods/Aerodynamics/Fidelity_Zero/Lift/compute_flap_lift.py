...
def compute_flap_lift(wing):
    ...
    dCL_aug   = wing.flaps.augmentation
    ...
    if bf_b != 0:
        if len(wing.Segments.keys())>0: 
            ...
            flap_chord[inb_ind+1:outb_ind] = cf
        ...
        for i in range(len(flap_defl)):
            if flap_defl[i] != 0:
                ...
                elif flap_type == 'single-slotted Fowler' or flap_type == 'double_slotted_fixed_vane':
                ...
                elif flap_type == 'double-slotted Fowler' or flap_type == 'triple-slotted Fowler':
        ...
        elif flap_type == 'single-slotted Fowler':
            eta = -8.5591e-6*df**3 + 2.2983e-4*df**2 - 3.1891e-3*df + 8.8817e-1
        elif flap_type == 'double-slotted fixed vane':
        ...
        dfCl0_av   = np.zeros(len(dfCl0)-1)
        S_segm_av  = np.zeros(len(dfCl0)-1)
        sweep_segm = np.zeros(len(dfCl0)-1)
        taper_segm = np.zeros(len(dfCl0)-1)
        ind = 0
        for i in range(len(dfCl0_av)):
            dfCl0_av[i]   = np.average([dfCl0[i],dfCl0[i+1]])*np.average(np.cos([loc_sweep[i],loc_sweep[i+1]]))
            sweep_segm[i] = np.average([loc_sweep[i],loc_sweep[i+1]])
            ind +=2
        ind = 0
        for i in range(len(S_segm)):
            #if S_segm[i] != 0:
            S_segm_av[i]  = S_segm[i]
            taper_segm[i] = taper[i]
            ind += 1

        ## Compute increase in CLmax
        dfCLmax = 0.92 * np.sum(np.multiply(dfCl0_av,S_segm_av))/(0.5 * ref_area) * dCL_aug

        ## Compute increase in CL0 
        # calculate Cl0 for any section
        dfCl0av = np.max(dfCl0_av)
        ...
        dfCL0 = dfCl0av * CLa/(2*np.pi) * Kc * Kb * dCL_aug     
