...
def compute_slat_lift(wing,w_Clmax):
    ...
    if bso - bsi != 0:
        if len(wing.Segments.keys())>0: 
            ...
            slat_chord[inb_ind+1:outb_ind] = cs
        ...
        # Estimate theoretical slat lift factor
        ...
        for i in range(len(dsCl0_av)):
            dsCl0_av[i] = np.average([dsCl0[i],dsCl0[i+1]])*np.average(np.cos([loc_sweep[i],loc_sweep[i+1]]))
            ind +=2
        ind = 0
