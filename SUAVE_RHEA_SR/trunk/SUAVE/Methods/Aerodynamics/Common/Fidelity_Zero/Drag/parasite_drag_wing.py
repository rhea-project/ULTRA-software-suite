...
def parasite_drag_wing(state,settings,geometry):
    ...
    # extra drag components
    CRUD = wing.CRUD
    misc = wing.miscellaneous
    IF   = wing.interference 
...
#         Sref                      = wing.areas.reference
    ...
    if num_segments>0:        
        ...
        for i_segs in range(num_segments):
            if i_segs == num_segments-1:
                ...
                xtu       = 0.5 * (wing.Segments[i_segs].transition_x_upper + wing.Segments[i_segs+1].transition_x_upper)
                xtl       = 0.5 * (wing.Segments[i_segs].transition_x_lower + wing.Segments[i_segs].transition_x_lower )    
                ...
                t_c_w = 0.5 * (wing.Segments[i_segs].thickness_to_chord + wing.Segments[i_segs+1].thickness_to_chord)
                                
                # compute wetted area of segment
                if geometry.outerwing.spans !=0:
                    Swet_seg = 1.72* Sref_seg                
                elif t_c_w < 0.05:
                ...
                segment_parasite_drag = (segment_parasite_drag*Sref_seg + misc) * IF * ( 1 + CRUD )
                
                total_wetted_area            += Swet_seg
                total_segment_parasite_drag  += segment_parasite_drag   
    ...
    # if wing has no segments      
        ...
        if geometry.outerwing.spans !=0:
            Swet = 1.72* wing.areas.reference
        elif t_c_w < 0.05:
        ...
        wing_parasite_drag = (wing_parasite_drag + misc) * IF * (1 + CRUD)
...
def parasite_drag_wing_from_surrogate(state,settings,geometry):
    """Computes the parasite drag due to wings
    ...
    # Unpack inputs
    state.conditions.aerodynamics.drag_breakdown.induced