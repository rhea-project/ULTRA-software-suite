...
#    x_axes = 'range'
    ...
    for segment in results.segments.values():
        ...
        Thrust_EM_max = segment.conditions.propulsion.thrust_breakdown.electric_max[:,0]/1000 
        Thrust_GT     = segment.conditions.propulsion.thrust_breakdown.jet[:,0]/1000 
        Thrust_GT_max = segment.conditions.propulsion.thrust_breakdown.jet_max[:,0]/1000 
        mdot          = segment.conditions.weights.vehicle_mass_rate[:,0]
        thrust        =  segment.conditions.frames.body.thrust_force_vector[:,0]
        sfc           = mdot/Thrust_GT *100000 # in e-5 kg/N/s #(mdot / Units.lb) / (thrust /Units.lbf) * Units.hr
        eta           = segment.conditions.propulsion.throttle[:,0]
        etaGT         = np.divide(Thrust_GT,Thrust_GT_max)
        etaEM         = np.divide(Thrust_EM,Thrust_EM_max)
        Dist          = segment.conditions.frames.inertial.position_vector[:,0]  / Units.nautical_miles
        if x_axes == 'range':
            axes = fig.add_subplot(2,1,1)
            axes.plot( Dist , Thrust , 'ro-', label='Total' )
            axes.plot( Dist , Thrust_EM , 'go-', label='Electric')
            axes.plot( Dist , Thrust_GT , line_style, label='Gas-turbine')
            axes.set_ylabel('Thrust (kN)',axis_font)
            axes.grid(True)

            axes = fig.add_subplot(2,1,2)
            axes.plot( Dist , etaGT , line_style, label='Total' )
            axes.plot( Dist , etaEM , 'go-', label='Electric' )
            axes.plot( Dist , eta , 'ro-', label='Gas-turbine' )
...
#            axes.grid(True)
    ...
    fid.write('  Time  |   h    |  Mach  |  Range |  Mass  |   CL   |   CD   |   L/D  | Thrust |  Drag  |  CDi   |  CDP   |  CDC   |  Oswald  |  sfc   |  sfc    |\n')
    ...
    for segment in results.segments.values():
        ...
        cdm = drag_breakdown.miscellaneous.total[:,0]
        cd  = drag_breakdown.total[:,0]
        e   = drag_breakdown.induced.efficiency_factor[:,0]
...
#        sfc_SI_str =   str('%9.5f'   % sfc_SI[0])     + '|'
        for i in range(len(time)):           
            ...
            cdc_str =   str('%8.5f'   % cdc[i])     + '|' 
            e_str   =   str('%8.5f'   % e[i])     + '|'
            sfc_str =   str('%8.5f'   % sfc[i])     + '|'
            sfc_SI_str =   str('%9.5f'   % sfc_SI[i])     + '|'        
            fid.write(time_str + h_str + Ma_str + Dist_str + mass_str + CL_str + CD_str + L2D_str + thrust_str + Drag_str + cdi_str + cdp_str + cdc_str + e_str + sfc_str + sfc_SI_str +'\n')
    fid.close
