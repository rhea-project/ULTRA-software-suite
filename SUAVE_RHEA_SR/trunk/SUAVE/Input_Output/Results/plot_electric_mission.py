...
# ----------------------------------------------------------------------
## @ingroup Input_Output-Results
def plot_electric_mission(case_name,results,x_axes='range', line_style='bo-'):
...
#    plt.savefig("LR_newDesign_drag_R.png")
    ...
    #   Battery Performance
    # ------------------------------------------------------------------
    fig = plt.figure("Battery performance",figsize=(8,10))
    '''for segment in results.segments.values():

        time     = segment.conditions.frames.inertial.time[:,0] / Units.min
        Dist = segment.conditions.frames.inertial.position_vector[:,0]  / Units.nautical_miles
        voltage   = segment.conditions.propulsion.voltage_under_load[:,0]
        torque    = segment.conditions.propulsion.motor_torque[:,0]
        current    = segment.conditions.propulsion.current[:,0] 
        
        if x_axes == 'range':
            axes = fig.add_subplot(3,1,1)
            axes.plot( Dist , voltage, line_style )
            axes.set_ylabel('Voltage under load (V)',axis_font)
            axes.grid(True)

            axes = fig.add_subplot(3,1,2)
            axes.plot( Dist , current, line_style )
            axes.set_xlabel('Range (NM)',axis_font)
            axes.set_ylabel('Current (A)',axis_font)
            axes.grid(True)    
        
            axes = fig.add_subplot(3,1,3)
            axes.plot( Dist , torque , line_style )
            axes.set_xlabel('Range (NM)',axis_font)
            axes.set_ylabel('Torque (Nm)',axis_font)
            axes.grid(True)
            #plt.savefig(case_name + "_velocity_R.pdf")
            plt.savefig(case_name + "_motor_data_R.png")            
            
            
        else:
            axes = fig.add_subplot(3,1,1)
            axes.plot( time , voltage, line_style )
            axes.set_ylabel('Voltage under load (V)',axis_font)
            axes.grid(True)

            axes = fig.add_subplot(3,1,2)
            axes.plot( time , current, line_style )
            axes.set_xlabel('Time (min)',axis_font)
            axes.set_ylabel('Current (A)',axis_font)
            axes.grid(True)    
        
            axes = fig.add_subplot(3,1,3)
            axes.plot( time , torque , line_style )
            axes.set_xlabel('Time (min)',axis_font)
            axes.set_ylabel('Torque (Nm)',axis_font)
            axes.grid(True)
            #plt.savefig(case_name + "_velocity_R.pdf")
            plt.savefig(case_name + "_motor_data_t.png")'''

    # ------------------------------------------------------------------
    #   Propeller performance
    # ------------------------------------------------------------------
    fig = plt.figure("Propeller performance",figsize=(8,10))
    for segment in results.segments.values():

        time  = segment.conditions.frames.inertial.time[:,0] / Units.min
        Dist  = segment.conditions.frames.inertial.position_vector[:,0]  / Units.nautical_miles
        Cp    = segment.conditions.propulsion.propeller_power_coefficient[:,0,None]
        omega = segment.conditions.propulsion.omega[:,0]
        
        if x_axes == 'range':
            axes = fig.add_subplot(2,1,1)
            axes.plot( Dist , omega , line_style )
            axes.set_ylabel('RPM',axis_font)
            axes.grid(True)
            
            axes = fig.add_subplot(2,1,2)
            axes.plot( Dist , Cp , line_style )
            axes.set_xlabel('Range (NM)',axis_font)
            axes.set_ylabel('Cp',axis_font)
            axes.grid(True)
            #plt.savefig(case_name + "_velocity_R.pdf")
            plt.savefig(case_name + "_Cp_R.png")
                       
        else:    
            axes = fig.add_subplot(2,1,1)
            axes.plot( time , omega , line_style )
            axes.set_ylabel('RPM',axis_font)
            axes.grid(True)
            
            axes = fig.add_subplot(2,1,2)            
            axes.plot( time , Cp , line_style )
            axes.set_xlabel('Time (min)',axis_font)
            axes.set_ylabel('Cp',axis_font)
            axes.grid(True)
            #plt.savefig(case_name + "_velocity_R.pdf")
            plt.savefig(case_name + "_Cp_t.png")
    ...
    fid.write('  Time  |  Segment |  h  |  Mach  |  Range |  Mass  |   CL   |   CD   |   L/D  | Thrust | Throttle | Drag  |  CDi   |  CDP   |  battery enegry   |\n')
    fid.write('   min  |     -    |  m  |    -   |   NM   |   kg   |   -    |   -    |   -    |   kN   |    -     |  kN   |   -    |   -    |     kWh / kg      |\n')
    for segment in results.segments.values():
        segm     = segment.tag
        time     = segment.conditions.frames.inertial.time[:,0] / Units.min
        Lift     = -segment.conditions.frames.wind.lift_force_vector[:,2]
...
#        sfc_SI_str =   str('%9.5f'   % sfc_SI[0])     + '|'
        for i in range(len(time)):           
            ...
            fid.write(time_str + segm + h_str + Ma_str + Dist_str + mass_str + CL_str + CD_str + \
                      L2D_str + thrust_str + throttle_str + Drag_str + cdi_str + cdp_str + energy_str +'\n')
    fid.close
