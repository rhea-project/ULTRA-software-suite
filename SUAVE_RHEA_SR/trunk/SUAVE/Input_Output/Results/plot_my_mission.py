...
# ----------------------------------------------------------------------
## @ingroup Input_Output-Results
def plot_my_mission(case_name,results,x_axes='range', line_style='bo-'):
...
#    plt.savefig("LR_newDesign_drag_R.png")
    ...
    for segment in results.segments.values():
        ...
        altitude = segment.conditions.freestream.altitude[:,0] * Units.m   #/ Units.ft
        mdot     = segment.conditions.weights.vehicle_mass_rate[:,0]
        thrust   =  segment.conditions.frames.body.thrust_force_vector[:,0]
        sfc      = (mdot / Units.lb) / (thrust /Units.lbf) * Units.hr
        Dist = segment.conditions.frames.inertial.position_vector[:,0] / Units.kilometer # / Units.nautical_miles
        if x_axes == 'range':
            axes = fig.add_subplot(2,1,1)
...
#            axes.grid(True)
    ...
    fid.write('  Time  |   h    |  Mach  |  Range |  Mass  |  AoA  |   CL   |   CD   |   L/D   |  Thrust |   Drag   |   CDi   |   CDP   |   sfc    |   sfc     |\n')
    fid.write('   min  |   m    |    -   |   NM   |   kg   |  deg  |   -    |   -    |    -    |    kN   |    kN    |    -    |    -    |   1/h    | e-5kg/N/s |\n')
    for segment in results.segments.values():
        time     = segment.conditions.frames.inertial.time[:,0] / Units.min
        Lift     = -segment.conditions.frames.wind.lift_force_vector[:,2]
...
#        sfc_SI_str =   str('%9.5f'   % sfc_SI[0])     + '|'
        for i in range(len(time)):           
            ...
            fid.write(time_str + h_str + Ma_str + Dist_str + mass_str + AoA_str + CL_str + CD_str + L2D_str + thrust_str + Drag_str + cdi_str + cdp_str + sfc_str + sfc_SI_str +'\n')
    fid.close
