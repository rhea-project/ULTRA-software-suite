...
# Constant_Accleration_Constant_Altitude_CAS.py
# 
# Created:  Jul 2014, S. Karpuk
# Modified: 
...
def initialize_conditions(segment):
    ...
    v0cas  = segment.calibrated_air_speed_start
    vfcas  = segment.calibrated_air_speed_end
    ...
    kcas0  = v0cas / Units.knots;       kcasf  = vfcas / Units.knots
    delta = pressure / pressure0 

    mach0 = 2.236*((((1+4.575e-7*kcas0**2)**3.5-1)/delta + 1)**0.2857 - 1)**0.5
    machf = 2.236*((((1+4.575e-7*kcasf**2)**3.5-1)/delta + 1)**0.2857 - 1)**0.5

    qc0  = pressure * ((1+0.2*mach0**2)**3.5 - 1)
    qcf  = pressure * ((1+0.2*machf**2)**3.5 - 1)

    eas0 = v0cas * (pressure/pressure0)**0.5*(((qc0/pressure+1)**0.286-1)/((qc0/pressure0+1)**0.286-1))**0.5
    easf = vfcas * (pressure/pressure0)**0.5*(((qcf/pressure+1)**0.286-1)/((qcf/pressure0+1)**0.286-1))**0.5
    
    v0 = eas0/np.sqrt(density/MSL_data.density[0])  
    vf = easf/np.sqrt(density/MSL_data.density[0])  
