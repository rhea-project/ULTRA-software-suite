...
def empty(vehicle):
    ...
    if settings == None:
        wt_factors = Data()
        wt_factors.main_wing = 0.0
        wt_factors.empennage = 0.0
        wt_factors.fuselage  = 0.0 

    else:
        wt_factors = Data()
        wt_factors.main_wing = vehicle.settings.weight_reduction_factors.main_wing
        wt_factors.empennage = vehicle.settings.weight_reduction_factors.empennage
        wt_factors.fuselage  = vehicle.settings.weight_reduction_factors.fuselage 
        Nult                 = vehicle.settings.load_factor # technology factor for load reduction
     
    num_seats                = vehicle.passengers
    bwb_aft_centerbody_area  = vehicle.wings['main_wing'].aft_centerbody_area # to be changed for compatibility issues!
    bwb_aft_centerbody_taper = vehicle.wings['main_wing'].aft_centerbody_taper
    bwb_cabin_area           = vehicle.wings['main_wing'].cabin_area
    ...
    if propulsor_name=='turbofan' or propulsor_name=='Turbofan':
        ...
        thrust_sls                       = propulsors.sealevel_static_thrust # thrust_sls - sea level static thrust of a single engine [Newtons]
    ...
    if vehicle.concept == 'BWB' or vehicle.concept == 'bwb':
        S_gross_w  = vehicle.wings['main_wing'].areas.reference - bwb_cabin_area - bwb_aft_centerbody_area # this one should be outer wing instead of total bwb
    else:
        S_gross_w  = vehicle.wings['main_wing'].areas.reference # for TAW it is the main wing directly
    ...
    elif vehicle.concept == 'BWB' or vehicle.concept == 'bwb':
        b          = vehicle.wings['main_wing'].outerwing.spans
        lambda_w   = vehicle.wings['main_wing'].outerwing.taper
        t_c_w      = vehicle.wings['main_wing'].outerwing.thickness_to_chord
        sweep_w    = vehicle.wings['main_wing'].outerwing.sweeps_quarter_chord
        mac_w      = vehicle.wings['main_wing'].outerwing.chords_mean_aerodynamic
        wing_c_r   = vehicle.wings['main_wing'].outerwing.chords_root
        S_h        = S_gross_w*0.01 # control surface area on bwb
        b 
        wt_wing    = wing_main.wing_main(S_gross_w,b,lambda_w,t_c_w,sweep_w,Nult,TOW,wt_zf)*(1.0-wt_factors.main_wing)
    ...
    wt_cabin           = cabin(bwb_cabin_area, TOW)*(1.0-wt_factors.fuselage)
    wt_aft_centerbody  = aft_centerbody(num_eng, bwb_aft_centerbody_area, bwb_aft_centerbody_taper, TOW)*(1.0-wt_factors.fuselage)
