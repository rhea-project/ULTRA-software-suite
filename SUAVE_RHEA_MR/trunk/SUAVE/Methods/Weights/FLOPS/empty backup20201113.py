...
def empty(vehicle):
    ...
    # Unpack and run EMWET if the flag is True
        if vehicle.wings['main_wing'].EMWET is True:
            ...
            wt_wing = weight_wing(1,TOW,WSR,S_gross_w,sweep_w,lambda_w,t_c_w,b,AR,GLOV, \
                                ...
                                eng_f,S_aftb,RSPCHD,RSPSOB,XLP,Nult,CAYF,cab_LE,var_sweep)
        #wt_wing    = wing_main.wing_main(S_gross_w,b,lambda_w,t_c_w,sweep_w,Nult,TOW,wt_zf)

        #wt_wing    = wt_wing*(1.-Wred)*(1+fold_ratio)    2020-11-02 already calculated in the weight_wing.py
    ...
    wt_paint   = weight_paint(paint_dens,Swet_w,Swet_h,Swet_v,S_fus,Swet_eng,Swet_can,Swet_fin)
    # -------------------------------------------------------------------------------------------------------    
