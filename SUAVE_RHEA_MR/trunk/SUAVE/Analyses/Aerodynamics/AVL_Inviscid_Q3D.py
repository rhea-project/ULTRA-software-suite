...
from SUAVE.Methods.Aerodynamics.Quasi_3D.write_input_deck import write_input_deck
...
# ----------------------------------------------------------------------
## @ingroup Analyses-Aerodynamics
class AVL_Inviscid_Q3D(Aerodynamics):
    ...
    def initialize(self,spanwise_vortices,chordwise_vortices):
        ...
        # delete all previous avl files  before the run
    ...
    def sample_training(self):
        ...
        for i,_ in enumerate(mach):
            for j,_ in enumerate(AoA):
                xy[i*len(AoA)+j,:] = np.array([AoA[j],mach[i]])
        for j,_ in enumerate(mach):
            ...
            # Obtain CD , CL and e
            CL[count*len(AoA):(count+1)*len(AoA),0]   = results.aerodynamics.lift_coefficient[:,0]
            CD[count*len(AoA):(count+1)*len(AoA),0]   = results.aerodynamics.drag_breakdown.induced.total[:,0]      
            e[count*len(AoA):(count+1)*len(AoA),0]    = results.aerodynamics.drag_breakdown.induced.efficiency_factor[:,0]  
...
# ----------------------------------------------------------------------
    ...
    def evaluate_conditions(self,run_conditions):
        ...
        Trefftz_output_template          = self.settings.filenames.Trefftz_output_template
        ...
        for case in cases:
            ...
            cases[case].Trefftz_result_filename = Trefftz_output_template.format(case)
